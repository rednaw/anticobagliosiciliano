...
status: pending
saved: 2026-09-11
overview: Git-backed /admin (Sveltia) for marketing copy. Maintainer first, then owners on GitHub. YAML in src/content/*.yml. Page titles on the page file. Shared chrome in chrome.yml.
---
...
- Hybrid: `src/content/*.yml` — houses, places, home, amenities, awards, contact, chrome (no email), arrive (labels only), imperdibili, privacy. Nested `{ it, en }` on each field. Sequence of houses/places is the import list in `src/lib/data/content.ts` (Vite plugin + `scripts/register-lib.mjs` both parse `.yml`).

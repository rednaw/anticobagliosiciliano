...
- **Surface:** two-line chip top-center with a Meteocons monochrome icon (Meteo attuale/Current weather, then temp · condition); Open-Meteo CC BY credit as its own bottom-left pill (OSM stays bottom-right). Chip and credit use site `--wash` / `--line` / `--ink` tokens (see [theme](theme.md)). Soft-fail → no chip/credit. No radar, no second map stack.

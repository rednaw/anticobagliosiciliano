...
- **Surface:** two-line chip top-center with a Meteocons monochrome icon (Meteo attuale/Current weather, then temp · condition); Open-Meteo CC BY credit as its own bottom-right pill left of OSM attribution (map visible in the gap). Soft-fail → no chip/credit. No radar, no second map stack.
